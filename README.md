...
4. 表示設定は横向き、解像度 `1280x720`、DPIは検証時の値から変更しない設定にします。解像度・表示倍率・縦横比を変更するとテンプレートやタップ座標が一致せず、安全停止することがあります。
...
本プロジェクトでは特定の数値（160/240/320など）を新たに設定するのではなく、テンプレートを取得したBlueStacksインスタンスのDPIをそのまま使います。DPIを変更するとゲーム内の文字・ボタン・レイアウトが変わり、同じ `1280x720` でも画像認識が失敗することがあります。Windows側の表示スケールではなく、BlueStacks内のAndroid表示密度を対象にしてください。
...
出力された `Physical density`（物理値）と `Override density`（上書き値）を記録し、検証時と一致することを確認します。`Override density` が表示されている場合は上書き設定が有効です。値が分からない状態で `wm density 160` などを実行して合わせるのではなく、BlueStacksの表示設定を検証時の状態へ戻し、アプリを再起動してから再確認してください。変更後は必ず画面テンプレートのpreflightを実行し、画面比率・主要ボタン・OCR領域が一致することを確認します。
