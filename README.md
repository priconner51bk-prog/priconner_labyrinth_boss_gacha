...
『プリンセスコネクト！Re:Dive』のラビリンスにおける、ボスガチャ処理を単体で管理・実行するプロジェクトです。
...
画面認識は座標・テンプレート・OCR 領域に依存するため、BlueStacks のウィンドウサイズ、DPI、ゲーム内表示倍率、縦横比を、検証済みの環境から変更しないでください。変更した場合は、実機実行前に画面テンプレートと ROI を再検証します。
