...
5. 画面テンプレートが不足していないことを確認します。OCRモデルの準備は不要です。
...
許容するボスはエリアごとに 1 つ以上指定します。`--passports N` は「1枚消費する試行を最大N回まで許可する」指定で、既定値は1000回です。試験時は `--passports 1` を指定します。結果は成功（`matched`）、失敗（`max_attempts`）、停止（`safety_stop`）に分かれます。
