...
許容するボスはエリアごとに 1 つ以上指定します。`--passports N` は「1枚消費する試行を最大N回まで許可する」指定で、既定値は100回です。1試行につきパスポートを1枚だけ消費し、N枚を一括消費することはありません。結果は成功（`matched`）、失敗（`max_attempts`）、停止（`safety_stop`）に分かれます。
